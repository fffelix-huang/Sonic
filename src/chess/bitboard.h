...
public:
	constexpr Square() {}
	constexpr Square(std::uint8_t num) : square(num) {}
	constexpr Square(int row, int col) : square(row * 8 + col) {}
	constexpr Square(Rank r, File f) : Square(f, r) {}
	constexpr Square(File f, Rank r) : square(static_cast<std::uint8_t>(r) * 8 + static_cast<std::uint8_t>(f)) {}
	...
	static bool is_valid(Square sq) {
		return is_valid(sq.row(), sq.col());
	}
	...
	friend constexpr Square operator+(Square sq, const Direction& d) {
		return sq += d;
	}
...
public:
	constexpr Bitboard() {}
	constexpr Bitboard(std::uint64_t bb) : board(bb) {}
	...
	struct iterator {
		...
		iterator(std::uint64_t bb) : bitboard(bb), sq(lsb(bb)) {}
